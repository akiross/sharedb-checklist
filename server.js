...
var db = require('sharedb-mongo')('mongodb://banana:ananas@localhost:27017/leest');
...
class Auth {
    constructor(path) {
        ...
        for (var i in list) {
            ...
            if (uid === null) {
                ...
                console.log("Allowing empty UIDs for anon:", this.allowEmptyUID);
            }
            ...
            if (access.includes('r'))
                this.readers[uid] = true;
            if (access.includes('m'))
                this.markers[uid] = true;
            if (access.includes('a'))
                this.adders[uid] = true;
            if (access.includes('d'))
                this.deleters[uid] = true;
        }
    }
    ...
    hasFullAccess(uid) {
        uid = this.toValidUID(uid);
        return this.canRead(uid) && this.canMark(uid) && this.canAdd(uid) && this.canDelete(uid);
    }

    makeToken(uid) {
        if (uid === null)
            return "00000000000000000000000000000000"; // Anon token
